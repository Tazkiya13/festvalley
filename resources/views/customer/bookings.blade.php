                            ...
                            <div class="card">
                                ...
                                <div class="card-body">
                                    ...
                                    <p class="card-text">
                                        ...
                                        @if($invoice->booking_status)
                                            <strong>Booking Status:</strong> 
                                            <span class="badge
                                                @if($invoice->booking_status == 'approved') badge-success
                                                @elseif($invoice->booking_status == 'pending') badge-warning
                                                @elseif($invoice->booking_status == 'rejected') badge-danger
                                                @else badge-secondary
                                                @endif
                                            ">{{ ucfirst($invoice->booking_status) }}</span>
                                        @endif
                                    </p>
                                    <a href="{{ route('booking.confirmation', $invoice->id) }}" class="btn btn-primary">View Details</a>
                                    @if($invoice->booking_status == 'approved' && $invoice->status == 'unpaid')
